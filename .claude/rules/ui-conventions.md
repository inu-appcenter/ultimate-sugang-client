...
- **원본 CSS 이식 프로젝트**(D2): shadcn/ui·sonner 미도입. 컴포넌트는 원본 클래스명(`perT`·`dataT`·`btn_blue`…)을 쓰고 `globals.css` 가 원§6 CSS 를 `var(--토큰)` 으로 치환해 담는다(`04 §4`). 다이얼로그는 `window.alert`/`confirm` 만. `border-radius` 전 요소 0. 폰트 스택 `Dotum, Gulim, 돋움, 굴림, arial, sans-serif`(웹폰트 없음 D18).
- **비밀번호 필드 금지**(`01 §1` 필수 1). 연습용 배너 상시 노출(D11).
...
| login-navy / login-divide / login-input / login-hint / btn-bg / etc-border | `bg-login-navy` `border-login-divide` `bg-login-input` `text-login-hint` `bg-btn-bg` `border-etc-border` | 동명 | 로그인 화면 |
| time-th / time-sun / time-sat | `bg-time-th` `bg-time-sun` `bg-time-sat` | 동명 | PRINT_APPLY(Q-5 전 미사용) |
...
- **컬럼 폭은 토큰이 아니다**(D10): `features/sukang/columns.ts` 의 설정 데이터(원§3) → `<colgroup>` `style={{width}}`.

## 글로벌 레이아웃 (`05 §1`)
- 사이드바·헤더 없음. 단일 `.wrap`(80%, min 1400px, padding 10px) 안에 위→아래: 배너 → `perT` → 탭 7(좌)+주의 문구(우) → 제목 라인(+검색, 조건 화면은 `.sjt_sch`) → 안내 영역(해당 화면만) → `.sch_areaT`(337px 스크롤) → `.regi_area`(헤더 문구 좌 + 출력 버튼 우 + 신청내역 테이블).
- 로그인: `#login` 928×380 절대 중앙(`05 §2`).
- 테이블: `table-layout: fixed`, 결과 영역만 내부 스크롤, 신청내역은 스크롤 없이 전건.
<!-- PROJECT:END -->
