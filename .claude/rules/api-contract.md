...
- 신청 검증 순서(D41): (CAPTCHA — `VITE_CAPTCHA=on` 시 D34 게이트, 기본 off) → 시간표 중복 `DUP_TIME`(반교시 단위 비교) → 동일 과목명 `DUP_SUBJECT`(시간 미겹침에도 차단, 독립) → 학점 상한 `CREDIT_EXCEEDED` → 정원 `CLASS_FULL`(프론트는 `isClosed` 행에 `마감` 버튼으로 **사전 차단 — 핸들러 없음**).
